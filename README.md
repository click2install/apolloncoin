...
Shell script to install a `Apollon Coin Masternode` on a Linux server running Ubuntu 16.04. Supports IPv4, IPv6 and multiple nodes on a single VPS.
...
```
wget -q https://github.com/click2install/apolloncoin/raw/master/install-apollon.sh  
bash install-apollon.sh
```
...
This script will work alongside masternodes that were installed by other means provided the masternode binaries `apollond` and `apollon-cli` are installed to `/usr/local/bin`.

Donations for the creation and maintenance of this script are welcome at:
&nbsp;

XAP: AUyScZXcpzgX7MFvgNAbuYRecftZEDifbG
...
The script assumes you are running a cold wallet on your local PC and this script will execute on a Ubuntu Linux VPS (server). The steps involved are:
 ...
 1. Run the masternode installation script as per the [instructions above](https://github.com/click2install/apolloncoin#installation).
...
You are now ready to configure your local wallet and finish the masternode setup
 ...
 1. Make sure you have downloaded the latest wallet from https://github.com/apollondeveloper/ApollonCore/releases
 ...
 5. Unlock your wallet and send **exactly 25,000 XAP** to the address created in step #4
 ...
 10. Your masternodes.conf file entry should look like: `MN-1 127.0.0.2:12218 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0` and it must be all on one line in your masternodes config file
...
The easiest ports to choose would be 12218 for your first masternode, then 12220, 12222, 12224, etc. The RPC port will be chosen based on your port selection. 
...
Once you have setup the 2nd or more masternodes, use the output of the script for each masternode and follow the [steps above](https://github.com/click2install/apolloncoin#how-to-setup-your-masternode-with-this-script-and-a-cold-wallet-on-your-pc) in your wallet, where each new masternode is a new line in your `masternode.conf` file. **NOTE:** All the port numbers in your masternode.conf file will be 12218 or the wallet will not re-start after you save the config file.

Note that multiple masternodes use only one instance of the `apollond` and `apollon-cli` binary files located in `/usr/local/bin` and they each have their own configuration located in `/home/<username>/.apolloncore` folder.
